...
class OBKBuildToolGUI(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.lbl_target = ttk.Label(frm_settings, text="", foreground="#555")
        self.lbl_target.grid(row=4, column=3, sticky="w", padx=(8, 8), pady=(6, 2))
...
# --- Build output ---
    ...
    def _on_platform_change(self):
        ...
        if hasattr(self, "lbl_target"):
            target = bt.get_target_platform(plat) if plat else ""
            self.lbl_target.config(text=(f"Target: {target}" if target else ""))
