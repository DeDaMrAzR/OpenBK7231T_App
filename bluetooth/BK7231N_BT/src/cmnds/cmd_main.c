...
static commandResult_t CMD_BLEScan(const void* context, const char* cmd, const char* args, int cmdFlags)
{
	int bOn;
	Tokenizer_TokenizeString(args, 0);
	if (Tokenizer_CheckArgsCountAndPrintWarning(cmd, 1))
	{
		ADDLOG_INFO(LOG_FEATURE_CMD, "BLEScan %d", HAL_BTScan_GetEnabled());
		return CMD_RES_NOT_ENOUGH_ARGUMENTS;
	}

	bOn = Tokenizer_GetArgInteger(0);
	HAL_BTScan_SetEnabled(bOn);
	ADDLOG_INFO(LOG_FEATURE_CMD, "BLEScan set to %d", bOn ? 1 : 0);
	return CMD_RES_OK;
}
...
void CMD_Init_Early() {
	...
	//cmddetail:{"name":"BLEScan","args":"[1or0]",
	//cmddetail:"descr":"Starts (1) or stops (0) BLE passive scan on supported platforms. Scan reports are printed in logs and published via MQTT as ble/adv.",
	//cmddetail:"fn":"CMD_BLEScan","file":"cmnds/cmd_main.c","requires":"",
	//cmddetail:"examples":"BLEScan 1"}
	CMD_RegisterCommand("BLEScan", CMD_BLEScan, NULL);
